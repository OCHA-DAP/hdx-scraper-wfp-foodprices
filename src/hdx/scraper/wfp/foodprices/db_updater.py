...
from hdx.utilities.dateparse import default_enddate, default_date
...
class DBUpdater:
    tables = {"DBCountry": DBCountry, "DBCommodity": DBCommodity, "DBMarket": DBMarket}

    def __init__(self, configuration: Configuration, session: Session):
        self._hxltags = configuration["hxltags"]
        self._session = session

    def update_tables(self, countryiso3: str, dbmarkets: List[DBMarket], time_period: Dict, hdx_url: str) -> None:
    ...
    def get_data_from_tables(self) -> Tuple[Dict, datetime, datetime]:
        ...
        table_data = {"DBCountry": {"rows": []}, "DBCommodity": {"rows": []}, "DBMarket": {"rows": []}}
